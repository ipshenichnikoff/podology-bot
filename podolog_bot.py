...
def main():
    ...
    client_conv = ConversationHandler(
        ...
        states={
            ST_CLIENT_MENU: [
                CallbackQueryHandler(client_callback),
            ],
            ST_CHOOSE_PROC: [
    ...
    admin_conv = ConversationHandler(
        ...
        states={
            ST_ADM_MENU: [
                CallbackQueryHandler(adm_callback),
            ],
            ST_ADM_CHOOSE_DATE: [
    ...
    # ── Глобальные fallback-обработчики ─────────────────────────────────────
    # Нужны когда бот перезапустился и ConversationHandler потерял состояние,
    # но пользователь жмёт кнопки из старых сообщений.
    app.add_handler(CallbackQueryHandler(client_callback, pattern="^my_apts$"))
    app.add_handler(CallbackQueryHandler(client_callback, pattern="^cancel_apt_"))
    app.add_handler(CallbackQueryHandler(client_callback, pattern="^back_main$"))
    app.add_handler(CallbackQueryHandler(client_callback, pattern="^about$"))
    app.add_handler(CallbackQueryHandler(client_callback, pattern="^book$"))
    app.add_handler(CallbackQueryHandler(client_callback, pattern="^repeat_proc$"))
    ...
    if jq:
        ...
        log.warning("JobQueue недоступен. "
